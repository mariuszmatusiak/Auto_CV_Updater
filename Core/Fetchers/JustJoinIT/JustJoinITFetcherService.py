...
POST_LOGIN_PAGE = "https://justjoin.it/"
...
class JustJoinITFetcherService(IJobsFetcherService):
    ...
    def _signIn(self, storeCookies=True):
        ...
        self.browser.visit(LOGIN_PAGE) # Visit login page and check if we get redirected to post login feed page
        if self.browser.getCurrentPageTitleAndUrl()["url"] == LOGIN_PAGE :
            if self.username is not None:
                logger.debug(f"LinkedIn Service initialized with the username {self.username}")
            elif os.getenv("JJIT_KEY"):
                logger.debug(f"Found a key for LinkedIn: {os.getenv("JJIT_KEY")}")
